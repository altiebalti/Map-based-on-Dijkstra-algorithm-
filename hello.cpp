...
int main(){
    ...
    roads.loadFromFile("roads1.png");
    transport.loadFromFile("public1.png");
    redPoint.loadFromFile("red.png");
    bluePoint.loadFromFile("blue.png");

    sf::Sprite backgroundRender(background); // Sprite - графическая единица
        ...
        if(pathSave.size()){
			...
			*/window.draw(line1, 10, sf::Lines);
			...
			for (int i = 1; i < pathSave.size(); i++){
				...
				window.draw(line, 10, sf::Lines); // если pathSave == 0, тогда просто соедини точки!
